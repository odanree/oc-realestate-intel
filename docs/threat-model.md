...
|---|---|---|---|---|---|---|---|
| T1 | Tampering / Information Disclosure | **Prompt injection via parcel query** — user crafts a query that exfiltrates the system prompt or coerces the summarize node to drop its provenance disclaimer | `POST /query` body | Hallucinated authoritative ownership claims; loss of honesty contract | H | RULE 6 in `SUMMARIZE_SYSTEM` is reinforced by the `_format_facts` NOTE block, which appends a non-overridable instruction to the *user* message context. Eval cases assert the disclaimer is present. **Runtime guard now lives in `DisclaimerCheck` of the `governance` LangGraph node** (see [ADR-0007](adr/0007-governance-node-with-incident-sink.md)) — backed by the standalone [`agent-governance`](https://github.com/odanree/agent-governance) package. **`PromptInjectionCheck` in the same node** reports matched injection patterns as `info` incidents so prevalence is trackable in Langfuse + (optionally) GitHub issues. Wiring coverage in `tests/test_governance_wiring.py`. | Tune injection patterns based on observed real-world fires. |
| T2 | Information Disclosure | **Output disclosure of training-data leakage** — model invents a plausible-looking OC government URL (`ocassessor.gov`) and presents it as authoritative | Summarize node | User clicks a fabricated URL; reputational harm | H | RULE 4 in `SUMMARIZE_SYSTEM` forbids inventing URLs/phone numbers; eval case originally surfaced this exact bug (see README "Three bugs the eval caught"). **`URLAllowlistCheck` in the `governance` node** (see [ADR-0007](adr/0007-governance-node-with-incident-sink.md)) extracts URLs from the answer and replaces any not on `GOVERNANCE_URL_ALLOWLIST` with `[URL removed by governance: not on allowlist]`. Recurring offending hostnames dedupe to one GitHub issue with a counter. | — |
...
| T13 | Information Disclosure / Privilege Escalation | **GitHubIssueSink content or token leakage** — the governance sink could (a) post sensitive query/answer content to a public issue tracker, or (b) misuse the `GITHUB_TOKEN` if the supplied scope is too broad | Governance node, on a fired check | PII leak; credential misuse | M | The `agent-governance` package's issue-body contract carries only check name, short detail, `trace_id`, fingerprint, timestamp — **never the raw user query or full answer**. Token usage is confined to `issues:write` on `GOVERNANCE_GITHUB_REPO`. Sink defaults to `LogSink` (no external calls) so misconfiguration in dev never leaks. | Document the required token scope (`issues:write`) explicitly in the deploy docs. Add a check-time assertion that `detail` strings in custom checks don't exceed a length cap (defensive — keeps future check authors from accidentally widening the leak surface). |
